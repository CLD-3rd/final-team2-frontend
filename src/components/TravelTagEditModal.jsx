...
const TravelTagEditModal = ({ onClose, userProfile, onSave }) => {
  ...
  const parseExistingTags = (existingTags) => {
    ...
    existingTags.forEach((tag) => {
      // 성향 태그 확인
      const personalityMatch = personalityOptions.find((opt) => opt.value === tag)
      if (personalityMatch) {
      ...
      // 여행지 태그 확인
      const destinationMatch = destinationOptions.find((opt) => opt.value === tag)
      if (destinationMatch) {
  ...
  const removePersonalityTag = (tagToRemove) => {
    setFormData((prev) => ({
      ...prev,
      selectedPersonalityTags: prev.selectedPersonalityTags.filter((tag) => tag !== tagToRemove),
    }))
  }
  ...
  const removeDestinationTag = (tagToRemove) => {
    setFormData((prev) => ({
      ...prev,
      selectedDestinationTags: prev.selectedDestinationTags.filter((tag) => tag !== tagToRemove),
    }))
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.travelStyle) {
      const selectedStyle = travelStyleOptions.find((opt) => opt.value === formData.travelStyle)
      if (selectedStyle) {
    ...
    if (formData.drinkingStyle) {
      const selectedDrinking = drinkingOptions.find((opt) => opt.value === formData.drinkingStyle)
      if (selectedDrinking) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="travel-tag-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="travel-tag-modal-body">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-section">
              ...
              <div className="personality-tags-container">
                {formData.selectedPersonalityTags.map((tag) => {
                  const option = personalityOptions.find((opt) => opt.value === tag)
                  return (
                    <button
                      key={tag}
                      type="button"
            ...
            {/* 음주 성향 */}
            <div className="form-section">
              <h3 className="section-title">여행 중 숙지사항에 대해 어떻게 생각하세요?</h3>
              <div className="radio-group">
                {drinkingOptions.map((option) => (
                  <label key={option.value} className="radio-item">
                    <input
                      ...
                      onChange={(e) => setFormData((prev) => ({ ...prev, drinkingStyle: e.target.value }))}
                    />
            ...
            <div className="form-section">
              ...
              <div className="destination-tags-container">
                {formData.selectedDestinationTags.map((tag) => {
                  const option = destinationOptions.find((opt) => opt.value === tag)
                  return (
                    <button
                      key={tag}
                      type="button"
